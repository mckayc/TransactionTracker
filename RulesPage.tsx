...
const RulesPage: React.FC<RulesPageProps> = ({ 
    ...
    const filteredRules = useMemo(() => {
        ...
        if (selectedDomain !== 'all') {
            list = list.filter(r => r.ruleCategory === selectedDomain || r.conditions.some(c => c.field === selectedDomain));
        }
    ...
    const handlePasteImport = async () => {
        ...
        if (hasApiKey() && (importText.length < 50 || !importText.includes(','))) {
            ...
            try {
                ...
                if (Array.isArray(healed)) rules = healed;
                else if (healed.rules) rules = healed.rules;
            } catch (e) {
        ...
        if (rules.length === 0) {
            rules = parseRulesFromLines(importText.split('\n'));
        }
        prepareDrafts(rules);
    };
