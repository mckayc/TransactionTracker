            ...
            <div className="flex-1 flex gap-6 min-h-0 overflow-hidden pb-10">
                ...
                <div className="w-80 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-0 flex-shrink-0">
                    <div className="p-3 border-b border-slate-100">
                        <div className="relative"><input type="text" placeholder="Search rules..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-4 py-2 bg-slate-50 border rounded-xl text-xs font-medium" /></div>
                    </div>
                    ...
                    <div className="p-3 border-t bg-slate-50 rounded-b-2xl"><button onClick={handleNew} className="w-full py-2 bg-indigo-600 text-white rounded-xl font-bold text-xs shadow-md">New Rule</button></div>
                </div>
                ...
                <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-0">
                    {(selectedRuleId || isCreating) ? (
                        <form onSubmit={handleSave} className="flex-1 flex flex-col min-h-0">
                            ...
                            <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                                <div className="grid grid-cols-3 gap-6">
                                    <div className="col-span-2 space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Rule Name</label><input type="text" value={ruleName} onChange={e => setRuleName(e.target.value)} className="w-full p-4 border rounded-2xl font-bold" required /></div>
                                ...
                                <div className="space-y-4"><h4 className="text-sm font-black text-slate-400 uppercase tracking-widest">Conditional Logic Tree</h4><div className="p-6 bg-slate-50 rounded-3xl border"><RuleBuilder items={conditions} onChange={setConditions} accounts={accounts} /></div></div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-slate-50/50">
                            <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-xl border mb-8 animate-bounce-subtle"><ShieldCheckIcon className="w-12 h-12 text-indigo-200" /></div>
                            <h3 className="text-2xl font-black text-slate-800">Master Rule Engine</h3>
                            <p className="text-slate-500 max-w-sm mt-4">Select a rule to refine, or use the AI Rule Creator to automatically detect merchant patterns from your statements.</p>
                        </div>
                    )}
                </div>
            </div>
