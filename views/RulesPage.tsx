...
const RulesPage: React.FC<RulesPageProps> = ({ 
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        setAiProposedRules(prev => prev.filter(p => p.id !== rule.id));
    };
    ...
    const toggleRuleExclusion = (id: string) => {
        setAiProposedRules(prev => prev.map(r => r.id === id ? { ...r, isExcluded: !r.isExcluded } : r));
    };
    ...
    const sourceContextDisplay = useMemo(() => {
        ...
        const descVals = conditions.filter(c => c.field === 'description' && c.value).map(c => c.value);
                ...
                <div className="bg-white border-2 border-indigo-100 rounded-3xl p-6 shadow-xl animate-fade-in space-y-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b pb-4 gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            {skippedDuplicateCount > 0 && (
                                <div className="px-3 py-1.5 bg-amber-50 text-amber-700 rounded-lg text-[10px] font-black uppercase flex items-center gap-2 border border-amber-100">
                                    <ExclamationTriangleIcon className="w-3 h-3" />
                                    {skippedDuplicateCount} Logical Duplicates Skipped
                                </div>
                            )}
                            {aiProposedRules.length > 0 && (
                                <button 
                                    onClick={handleAcceptAllProposed}
                                    className="px-6 py-2 bg-indigo-600 text-white font-black rounded-xl text-[10px] uppercase shadow-lg hover:bg-indigo-700 transition-all flex items-center gap-2"
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 overflow-y-auto max-h-[500px] custom-scrollbar">
                            ...
                            {aiProposedRules.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-slate-300 py-10"><TableIcon className="w-10 h-10 mb-2 opacity-20" /><p className="text-sm font-bold">No new logic patterns detected.</p></div>
                            ) : (
                                <div className="space-y-3">
                                    {aiProposedRules.map((r, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className={`bg-white p-4 rounded-xl border transition-all shadow-sm space-y-3 animate-fade-in group/card ${r.isExcluded ? 'opacity-40 grayscale border-slate-200' : 'border-indigo-100 hover:border-indigo-400'}`}>
                                                <div className="flex justify-between items-start">
                                                    <div className="flex-1 min-w-0 pr-4">
                                                        <div className="flex items-center gap-2">
                                                            <h5 className={`text-sm font-bold text-slate-800 truncate ${r.isExcluded ? 'line-through' : ''}`}>{r.name}</h5>
                    ...
                    {(selectedRuleId || isCreating) ? (
                        <form onSubmit={handleSave} className="flex-1 flex flex-col min-h-0 animate-fade-in">
                            ...
                            <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar bg-white">
                                ...
                                <div className="space-y-4">
                                  <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">Conditional Logic Tree</h4>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isAddingNewCategory ? (
                                                <div className="animate-fade-in bg-purple-50 p-3 rounded-xl border-2 border-purple-200">
                                                    <input type="text" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder="New Category Name..." className="w-full p-2 bg-white text-xs font-bold border-purple-300 outline-none" />
                                                </div>
                                            ) : (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isAddingNewPayee ? (
                                                <div className="animate-fade-in bg-blue-50 p-3 rounded-xl border-2 border-blue-200">
                                                    <input type="text" value={newPayeeName} onChange={e => setNewPayeeName(e.target.value)} placeholder="New Payee Name..." className="w-full p-2 bg-white text-xs font-bold border-blue-300 outline-none" />
                                                </div>
                                            ) : (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isAddingNewMerchant ? (
                                                <div className="animate-fade-in bg-orange-50 p-3 rounded-xl border-2 border-orange-200">
                                                    <input type="text" value={newMerchantName} onChange={e => setNewMerchantName(e.target.value)} placeholder="New Merchant Name..." className="w-full p-2 bg-white text-xs font-bold border-orange-300 outline-none" />
                                                </div>
                                            ) : (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isAddingNewLocation ? (
                                                <div className="animate-fade-in bg-emerald-50 p-3 rounded-xl border-2 border-emerald-200">
                                                    <input type="text" value={newLocationName} onChange={e => setNewLocationName(e.target.value)} placeholder="New Location (City, ST)..." className="w-full p-2 bg-white text-xs font-bold border-emerald-300 outline-none" />
                                                </div>
                                            ) : (
