...
/* Fixed: Added missing TableIcon import */
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                {appState === 'idle' ? (
                    <div className="flex flex-col h-full overflow-hidden">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex p-1 bg-slate-100 rounded-xl">
                                <button onClick={() => setImportMethod('upload')} className={`px-3 py-1.5 text-[10px] font-black rounded-lg transition-all ${importMethod === 'upload' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>FILE</button>
                                <button onClick={() => setImportMethod('paste')} className={`px-3 py-1.5 text-[10px] font-black rounded-lg transition-all ${importMethod === 'paste' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>TEXT</button>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                        ...
                        <p className="font-black text-slate-800">{progressMessage}</p>
                        <p className="text-xs text-slate-400 mt-2">Gemini is synthesizing patterns...</p>
                    </div>
                ) : (
