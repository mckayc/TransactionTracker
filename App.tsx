...
// Fixed: Using the correct IntegrationsPage component located in views/
        ...
        <div className="flex h-screen bg-slate-50 overflow-hidden font-sans relative">
            ...
            <main className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${isSidebarCollapsed ? 'ml-20' : 'ml-64'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 relative custom-scrollbar bg-slate-50/50">
                    ...
                    {/* Fixed: Handled 'integration-video-product-joiner' view which was missing from App.tsx logic */}
                    {currentView === 'integration-video-product-joiner' && (
                        <div className="bg-white p-12 rounded-[3rem] text-center border border-slate-200 shadow-sm animate-fade-in">
                            <SparklesIcon className="w-16 h-16 text-indigo-400 mx-auto mb-6" />
                            <h2 className="text-3xl font-black text-slate-800">Video ROI Engine</h2>
                            <p className="text-slate-500 mt-4 max-w-lg mx-auto">This module is being synthesized. It will allow you to link YouTube video IDs directly to Amazon ASIN performance for true conversion attribution.</p>
                            <button onClick={() => setCurrentView('integrations')} className="mt-8 px-8 py-3 bg-indigo-600 text-white font-black rounded-2xl">Return to Hub</button>
                        </div>
                    )}
                </div>
            </main>
