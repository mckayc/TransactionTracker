        ...
        return (
            <g key={item.label + i} className="group/slice">
                ...
                {style === 'labeled' && percentage > 0.05 && (
                    <text x={lx} y={ly} fill="white" textAnchor="middle" alignmentBaseline="middle" className="pointer-events-none text-[8px] font-black drop-shadow-sm select-none">
                ...
                {style === 'callout' && percentage > 0.03 && (
                    <g className={`transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-40'}`}>
                        <line x1={px} y1={py} x2={lx} y2={ly} stroke={item.color} strokeWidth="1" strokeDasharray="2,2" />
        ...
        <div className="select-none">
            ...
            >
                <div className="w-4 flex justify-center flex-shrink-0">
                    {hasChildren && (
                        <button onClick={(e) => { e.stopPropagation(); onToggleExpand(node.id); }} className="p-0.5 rounded text-slate-400 hover:text-indigo-600 hover:bg-white">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                        <span className={`text-[11px] font-bold truncate pr-2 ${isHidden ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{node.label}</span>
                        <span className={`text-[11px] font-black font-mono ${isHidden ? 'text-slate-200' : 'text-slate-900'}`}>{isHidden ? '••••••' : formatCurrency(node.value)}</span>
                    </div>
            ...
            {isExpanded && hasChildren && (
                <div className="mt-0.5">
                    {node.children.sort((a,b) => b.value - a.value).map(child => (
                        <BreakdownRow key={child.id} node={child} total={total} maxVal={maxVal} onToggleVisibility={onToggleVisibility} onInspect={onInspect} hiddenIds={hiddenIds} expandedIds={expandedIds} onToggleExpand={onToggleExpand} depth={depth + 1} />
                    ))}
                </div>
            )}
        </div>
...
export const CashFlowWidget: React.FC<Props> = ({ 
    widget, transactions, categories, counterparties, accounts, transactionTypes, users, onUpdateConfig 
}) => {
    ...
    const handleShiftPeriod = (direction: 'prev' | 'next') => {
        onUpdateConfig({ ...config, lookback: direction === 'prev' ? lookbackUnits + 1 : Math.max(0, lookbackUnits - 1) });
    };
    ...
    const tree = useMemo(() => {
        ...
        const getOrCreateNode = (id: string, label: string, parentId?: string): BreakdownNode => {
            if (!nodeMap.has(id)) {
                ...
                if (parentId) {
                    if (dataType === 'category') { const p = categories.find(c => c.id === parentId); if (p) getOrCreateNode(p.id, p.name, p.parentId); }
                    else if (dataType === 'counterparty') { const p = counterparties.find(c => c.id === parentId); if (p) getOrCreateNode(p.id, p.name, p.parentId); }
                }
            }
        ...
        transactions.forEach(tx => {
            ...
            if (txDate >= start && txDate <= end && !tx.isParent) {
                ...
                if (dataType === 'category') { const cat = categories.find(c => c.id === tx.categoryId); key = tx.categoryId; label = cat?.name || 'Unallocated'; parentId = cat?.parentId; }
                else if (dataType === 'counterparty') { const cp = counterparties.find(c => c.id === tx.counterpartyId); key = tx.counterpartyId || `desc_${tx.description}`; label = cp?.name || tx.description || 'Unknown Entity'; parentId = cp?.parentId; }
                else if (dataType === 'account') { key = tx.accountId; label = accounts.find(a => a.id === key)?.name || 'Unknown Account'; }
                else { key = tx.typeId; label = txType?.name || 'Other'; }

                if (keywords.length > 0 && keywords.some(kw => `${tx.description} ${tx.originalDescription || ''} ${label}`.toLowerCase().includes(kw))) return;
                if (excludeUnknown && (key === 'unknown' || label === 'Unallocated' || label === 'Unknown Entity' || tx.categoryId === 'cat_other')) return;
        ...
        const calculateAggregate = (node: BreakdownNode): number => {
            ...
            node.children.forEach(c => { sum += calculateAggregate(c); node.transactions = [...node.transactions, ...c.transactions]; });
            node.value = sum; return sum;
        };
        const sortAndColorRecursive = (node: BreakdownNode, baseColor: string) => {
            node.color = baseColor;
            node.children.sort((a, b) => b.value - a.value).forEach(c => sortAndColorRecursive(c, baseColor));
        };

        roots.forEach(calculateAggregate);
        const activeRoots = roots.filter(r => Math.abs(r.value) > 0).sort((a, b) => b.value - a.value);
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            <div className="px-6 py-4 flex items-center justify-between flex-shrink-0 bg-white border-b border-slate-50">
                <button onClick={() => handleShiftPeriod('prev')} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors"><ChevronLeftIcon className="w-5 h-5" /></button>
                <div className="px-5 py-2 bg-slate-900 rounded-full shadow-lg"><p className="text-[10px] font-black text-indigo-400 uppercase tracking-tighter text-center">{displayLabel}</p></div>
                <button onClick={() => handleShiftPeriod('next')} disabled={lookbackUnits === 0} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors disabled:opacity-0"><ChevronRightIcon className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 p-6 flex flex-col min-h-0">
                <div className="flex-shrink-0 mb-6">
                    {vizType === 'cards' ? (
                        <div className="p-6 bg-slate-900 rounded-[2rem] text-white relative overflow-hidden shadow-xl">
                            <div className="relative z-10"><p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">In-Period Context</p><p className="text-3xl font-black">{formatCurrency(totalVisible)}</p></div>
                            <TrendingUpIcon className="absolute -right-4 -bottom-4 w-24 h-24 text-white opacity-5" />
                        </div>
                    ) : (
                        <DynamicArcChart data={visibleTree.map(n => ({ label: n.label, value: n.value, color: n.color, transactions: n.transactions }))} total={totalVisible} style={pieStyle} onSegmentClick={(segment) => setInspectingNode(segment as any)} />
                    )}
                </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[200] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
                        <div className="p-8 border-b bg-slate-50 flex justify-between items-center">
                            <div className="flex items-center gap-4"><div className="p-3 rounded-2xl bg-white shadow-sm" style={{ color: inspectingNode.color }}><TableIcon className="w-6 h-6" /></div><div><h3 className="text-2xl font-black text-slate-800">{inspectingNode.label}</h3><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">{inspectingNode.transactions.length} entries totaling {formatCurrency(inspectingNode.value)}</p></div></div>
                        ...
                        <div className="flex-1 overflow-hidden relative"><TransactionTable transactions={inspectingNode.transactions} accounts={accounts} categories={categories} counterparties={counterparties} users={users} tags={[]} transactionTypes={transactionTypes} onUpdateTransaction={() => {}} onDeleteTransaction={() => {}} visibleColumns={new Set(['date', 'description', 'account', 'amount'])} /></div>
                        <div className="p-6 border-t bg-white flex justify-end"><button onClick={() => setInspectingNode(null)} className="px-8 py-3 bg-slate-100 text-slate-600 font-black rounded-xl hover:bg-slate-200 uppercase text-[10px] tracking-widest">Close Inspector</button></div>
                    </div>
                </div>
            )}
        </div>
